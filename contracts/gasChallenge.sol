...
contract gasChallenge {
    ...
    assembly {
        // Load the current storage slot for the "numbers" array
        let numbersSlot := sload(numbers.slot)

        // Set all 10 elements to 0 using sstore
        for { let i := 0 } lt(i, 10) { i := add(i, 1) } {
            sstore(mul(numbersSlot, i), zero)
        }
    }
}
 }
